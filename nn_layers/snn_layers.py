...
""" Pytorch implementation of Spiking neurons and layers, full code is available at https://github.com/kamata1729/FullySpikingVAE 
Hiromichi Kamata, et al. 'Fully Spiking Variational Autoencoder' https://arxiv.org/pdf/2110.00375v3.pdf 
"""
...
class LIFSpike(nn.Module):
    ...
    def forward(self, x):
        ...
        for step in range(nsteps):
            u, out[..., step] = self.state_update(
                u, out[..., max(step-1, 0)], x[..., step])
        return out
...
class tdLinear(nn.Linear):
    def __init__(self,
                 ...
                 bn=None,
                 spike=None):
        assert type(in_features) == int, 'inFeatures should not be more than 1 dimesnion. It was: {}'.format(
            in_features.shape)
        assert type(out_features) == int, 'outFeatures should not be more than 1 dimesnion. It was: {}'.format(
            out_features.shape)
...
class tdConv(nn.Conv3d):
    def __init__(self,
        ...
        if type(kernel_size) == int:
            ...
            raise Exception('kernelSize can only be of 1 or 2 dimension. It was: {}'.format(
                kernel_size.shape))
        ...
        if type(stride) == int:
            ...
            raise Exception(
                'stride can be either int or tuple of size 2. It was: {}'.format(stride.shape))
        ...
        if type(padding) == int:
            ...
            raise Exception(
                'padding can be either int or tuple of size 2. It was: {}'.format(padding.shape))
        ...
        if type(dilation) == int:
            ...
            raise Exception(
                'dilation can be either int or tuple of size 2. It was: {}'.format(dilation.shape))
...
class tdConvTranspose(nn.ConvTranspose3d):
    def __init__(self,
        ...
        if type(output_padding) == int:
            ...
            raise Exception(
                'output_padding can be either int or tuple of size 2. It was: {}'.format(padding.shape))
    ...
    def forward(self, input):
        ...
        if self.affine:
            input = input * self.weight[None, :, None, None,
                                        None] + self.bias[None, :, None, None, None]
...
class MembraneOutputLayer(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        arr = torch.arange(params.n_steps-1, -1, -1)
        self.register_buffer("coef", torch.pow(0.8, arr)[
...
class tdConv2d(nn.Conv2d):
    def __init__(self,
        ...
        if type(kernel_size) == int:
            ...
            raise Exception(f'kernelSize can only be of 1 dimension. It was: {kernel_size.shape}')
        ...
        if type(stride) == int:
            ...
            raise Exception(f'stride can be int only. It was: {stride.shape}')

        # padding
        if type(padding) == int:
            padding = (padding, 0)
        else:
            raise Exception(f'padding can be either int only. It was: {padding.shape}')

        # dilation
        if type(dilation) == int:
            dilation = (dilation, 1)
        else:
            raise Exception(f'dilation can be either int only. It was: {dilation.shape}')

        super(tdConv2d, self).__init__(in_channels, out_channels, kernel, stride, padding, dilation, groups,
                                       bias=bias)
        self.bn = bn
        self.spike = spike
        self.is_first_conv = is_first_conv
...
class tdConvTranspose2d(nn.ConvTranspose2d):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
        ...
        if type(padding) == int:
            ...
            raise Exception(f'padding can be int only. It was: {padding.shape}')

        # dilation
        if type(dilation) == int:
            dilation = (dilation, 1)
        else:
            raise Exception(f'dilation can be int only. It was: {dilation.shape}')

        # output padding
        if type(output_padding) == int:
            output_padding = (output_padding, 0)
        else:
            raise Exception(f'output_padding can be int only. It was: {padding.shape}')
    ...
    def forward(self, x):
        x = F.conv_transpose2d(x, self.weight, self.bias,
                               self.stride, self.padding,
                               self.output_padding, self.groups, self.dilation)
